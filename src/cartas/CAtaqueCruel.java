 ...
 * @author paulo
 */
public class CAtaqueCruel extends Carta {

    public CAtaqueCruel(String Cor, String Simbolo) {
        super(Cor, Simbolo);
    }
    ...
    public void descartar(Jogo j) {
        if (j.getBaralho().topoMonte().getCor().equals(this.getCor()) || j.getBaralho().topoMonte().getSimbolo().equals(this.getSimbolo())) {
            ...
            j.getJogadores().get(j.jogadorProximo()).ReceberCarta(j.getBaralho().topoDeque());
            j.proximo();
            System.out.println("O jogador " + j.getJogadorAtivo().getNome() + " comprou quatro cartas e foi pulado.");
